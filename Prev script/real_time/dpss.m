...
%   $Revision: 1.9.4.6 $  $Date: 2007/12/14 15:04:22 $

% Input parsing and validation
error(nargchk(2,6,nargin,'struct'));
[method,k,Ni,traceFlag,err_msg] = parseinputs(N,NW,varargin{:});
if ~isempty(err_msg)
    error(generatemsgid('invalidInput'),err_msg);
end
...
case {'spline','linear'}
   ...
   if ~isempty(err_msg)
       error(generatemsgid('invalidTimeBandwidthProduct'),err_msg);
   end
...
for j = 1:Lv
   ...
   [msg2,id2] = lastwarn('');
   if ~isempty(msg2)
       warning(generatemsgid('DPSS'), 'Warning occurred for sequence %d \n',j);
       lastwarn('');
       msg = msg2;
   end
   E(:,j) = e/norm(e);
...
if strcmp(int, 'linear') && N > M
  error(generatemsgid('invalidInput'),...
      'Linear interpolation cannot be used for N>Ni. Use splining instead.');
end
...
if nargin<=4
    ...
    if size(E,1)~=M
        error(generatemsgid('invalidInput'),...
            'Ni and row size of E don''t match.');
    end
end
...
if ~isnumeric(N) || N < 1 || ~isfinite(N) || rem(N,1)
    err_msg = 'The length of the sequence, N, must be a positive integer.';
    return;
end

if ~isnumeric(NW) || NW < 0 || ~isfinite(NW)
    err_msg = 'Time-bandwidth product NW must be a positive number.';
    return;
end
