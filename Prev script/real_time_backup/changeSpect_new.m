    ...
    if ~isempty(dat)
        ...
        if((col==250))
            ...
            if (count > 1)
               ...
               clear dPowerAllElec;
               dPowerAllElec = squeeze(10*double(log10(power(count,:,:))-repmat(mLogBL,1,1,size(power,3))));
               %dPower = [dPower; log10((power(count,:))) - mLogBL];
                ...
                %pause(0.1)
                %cla(handles.axes2)
                caxis(handles.axes2,[-40 40]);
                 ylim(handles.axes2, [0 120]);
                colorbar('peer',hRawSpectrum);
            end
            if (count ==1)
...
%                 realTimeDataAnalysis_new(handles, adnew, bd, gd,in);
%                 % pause(0.01);
%             end
            end
