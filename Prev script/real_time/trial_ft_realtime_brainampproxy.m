function trial_ft_realtime_brainampproxy(cfg)
...
while isempty(hdr)
  ...
  msg       = [];
  msg.uid   = tcpread_new(sock, 16, 'uint8');
  msg.nSize = tcpread_new(sock, 1, 'int32');
  msg.nType = tcpread_new(sock, 1, 'int32');
%msg.nType=1;
  if feedback
    fprintf('msg.nType = %d\n', msg.nType);
  end
  ...
  switch msg.nType
    case 1
      ...
      msg.nChannels         = tcpread_new(sock, 1, 'int32');
      msg.dSamplingInterval = tcpread_new(sock, 1, 'double');
      msg.dResolutions      = tcpread_new(sock, msg.nChannels, 'double');
      for i=1:msg.nChannels
        msg.sChannelNames{i} = tcpread_new(sock, char(0), 'char');
      end
...
while (true)
  ...
  switch msg.nType
    case 2
      ...
      msg.nBlocks       = tcpread_new(sock, 1, 'int32');
      msg.nPoints       = tcpread_new(sock, 1, 'int32');
      msg.nMarkers      = tcpread_new(sock, 1, 'int32');
      msg.nData         = tcpread_new(sock, [msg.nChannels msg.nPoints], 'int16');
      for i=1:msg.nMarkers
        msg.Markers(i).nSize      = tcpread_new(sock, 1, 'int32');
        msg.Markers(i).nPosition  = tcpread_new(sock, 1, 'int32');
        msg.Markers(i).nPoints    = tcpread_new(sock, 1, 'int32');
        msg.Markers(i).nChannel   = tcpread_new(sock, 1, 'int32');
        msg.Markers(i).sTypeDesc  = tcpread_new(sock, char(0), 'char');
      end
    ...
    case 4
      ...
      msg.nPoints       = tcpread_new(sock, 1, 'int32');
      msg.nMarkers      = tcpread_new(sock, 1, 'int32');
      msg.fData         = tcpread_new(sock, [msg.nChannels msg.nPoints], 'single');
      for i=1:msg.nMarkers
        msg.Markers(i).nSize      = tcpread_new(sock, 1, 'int32');
        msg.Markers(i).nPosition  = tcpread_new(sock, 1, 'int32');
        msg.Markers(i).nPoints    = tcpread_new(sock, 1, 'int32');
